...
use NativeCall;
use PDF::Native::Defs :libpdf, :types;

sub pdf_write_bool(PDF_TYPE_BOOL $val, Blob[uint8] $buf, size_t $buf-len)
    returns size_t
    is native(libpdf) {*};

sub pdf_write_int(PDF_TYPE_INT64 $val, Blob[uint8] $buf, size_t $buf-len)
    returns size_t
    is native(libpdf) {*};

sub pdf_write_real(PDF_TYPE_REAL $val, Blob[uint8] $buf, size_t $buf-len)
    returns size_t
    is native(libpdf) {*};
...
sub pdf_write_xref_seg(PDF_TYPE_XREF $val, size_t $rows, Blob[uint8] $buf, size_t $buf-len)
    returns size_t
    is native(libpdf) {*};

sub pdf_write_name(PDF_TYPE_CODE_POINTS $val, size_t $val-len, Blob[uint8] $buf, size_t $buf-len)
    returns size_t
    is native(libpdf) {*};
...
method write-name(Str:D $val, Blob $buf? is copy --> Str) {
    my PDF_TYPE_CODE_POINTS $in .= new: $val.ords;
    my \quads = $in.elems;
