...
sub do-packing($in is copy, $n, $m, &pack) {
    my uint32 $in-len = $in.elems;
    $in = Blob[$n.&container].new($in)
        unless $in.isa(Blob);
    my $out := Blob[$m.&container].allocate(($in-len * $n + $m-1) div $m);
    $in.&pack($out, $in-len);
    $out;
}
...
multi sub unpack( $in!, Array $W! --> array)  {
    ...
    $in-buf.&pdf_buf_unpack_W_64( $out-buf, $in-len, $W-buf, +$W);
...
multi sub pack( $in, Array $W! --> Blob)  {
    ...
    $in-buf.&pdf_buf_pack_W_64($out, $out-len, $W-buf, +$W);
    $out;
}
...
our sub packing-widths($in, UInt:D $n) is export(:pack) {
    ...
    my blob64 $in-buf = $in ~~ blob64 ?? $in !! blob64.new($in);
    $in-buf.&pdf_buf_pack_compute_W_64(+$in-buf, $W-buf, $n);
    $W-buf.List;
}
...
our sub unpack-xref-stream($in, $index) is export(:pack-xref-stream) {
    ...
    my $n = $in-buf.&pdf_buf_unpack_xref_stream($out-buf, $rows, $index-buf, +$index-buf);
    given $n <=> $rows {
...
our sub pack-xref-stream($in, @out, @index) is export(:pack-xref-stream) {
    ...
    my size_t $index-len;
    my $size = $in-buf.&pdf_buf_pack_xref_stream($out-buf, $rows, $index-buf, $index-len);
    @out Z= $out-buf;
